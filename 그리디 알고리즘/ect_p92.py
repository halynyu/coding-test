...
이 경우 특정한 인덱서의 수가 연속해서 세 번까지만 더해질 수 있으므로 큰 수의 법칙에 따른 결과는 6+6+6+5+6+6+6+5인 46이 된다.
...
# 이제 가장 큰 수부터 k번씩 더하면 되겠다
result = 0

while True:
    for _ in range(k):
        # 더해야 하는 수(m)가 0이면 반복문 탈출
        if m == 0:
            break
        # 가장 큰 수를 result에 더해주면서, m은 1씩 감소
        result += arr[n-1]
        m -= 1
    if m == 0:
        break
...
# 이코테 책에는 두 번째로 큰 숫자까지 더해지는 경우만 생각하는 것 같은데 왜??
# 추가적으로 코드 더 생각해보기
